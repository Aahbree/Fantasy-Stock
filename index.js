...
function getDetails(){
  ...
  function createDashboard(result){
    ...
    //   //append rank
      ...
      scoreDiv.setAttribute('class', 'subdetail2');
      scoreDiv.innerHTML="Score in this competition : "+score.toString();
      ...
      competition.appendChild(scoreDiv);
...
function showCompetitions(res) {
  if (res.length == 0) {
    ...
    for (var i = 0; i < results.length; i++) {
       ...
       //setting onclick
       if((results[i]['status']==='LIVE')||(results[i]['status']==='Live')||(results[i]['status']==='CLOSED')||(results[i]['status']==='Closed')){
          if((results[i]['status']==='LIVE')||(results[i]['status']==='Live')){
             newDiv.setAttribute('onclick', "util();");
          }else{
             newDiv.setAttribute('onclick', "util2();");
          }
        }else{
       ...
       competion_name.className='competition_name_tag';
  ...
  function util2(){
    alert("Sorry, the competition has ended");
  }
  function util(){
...
function showCompetitionDetails(res) {
  if (res.length == 0) {
    ...
    var companies_list=document.createElement("div");
    companies_list.setAttribute("class", "company_list");
    ...
    for (var i = 0; i < 300; i++) {
        ...
        slot.appendChild(companySlot);
        slot.innerHTML+=results[i];

        companies_list.append(slot);
        form.append(companies_list);
    }
